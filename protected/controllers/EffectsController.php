	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('create','update','reportScriptMapping'),
				'users'=>array('@'),
			),
...
function addToQueue($formId, $fieldId, $effectId, $triggerId) {
    ...
    $queueFilename = 'C:\xampp\htdocs\report\AjaxFiles\effectScriptsQueue.json';
...
//                $filename = 'C:\xampp\htdocs\report\AjaxFiles\effectScripts.json';
//
...
public function actionCreate()
{
    ...
    if (isset($_POST['Effects'])) {
        ...
        if (!$queueCheck) {
            ...
            // Check if triggerId or fieldId is null
            if ($triggerId === null || $fieldId === null) {
                // If triggerId or fieldId is null, save data in the database and return
                if ($model->save()) {
                    $this->redirect(array('view', 'id' => $model->id));
                } else {
                ...
                $scriptCode = array(); // Default value in case decoding fails
                if ($scriptCodeModel instanceof ScriptCode) {
                    $decodedCode = json_decode($scriptCodeModel->code, true);
                    if ($decodedCode !== null) {
                        $scriptCode = $decodedCode;
                    } else {
                        // Handle the case where JSON decoding fails (e.g., log an error)
                        // For now, let's just use an empty array as the default value
                        $scriptCode = array();
                    }
                }
                ...
                if ($triggerModel instanceof EffectTrigger) {
                    $triggerName = $triggerModel->effect_trigger;
                } else {
                    // Handle the case where $triggerModel is not an object (e.g., provide a default value or log an error)
                    // For now, let's just set an empty string as the default value
                    $triggerName = '';
                }
...
//    $actionId = "testReport";
    ...
    if ($applicationForm) {
        ...
        if ($effectModel) {
            ...
            $scriptCode = ScriptCode::model()->findByPk($scriptCodeId);

            if ($scriptCode) {
                $customScript = $scriptCode->code;
//                
              $jsonObject = json_decode($customScript);
//              print_r($customScript);
//                die();
//              $jsCode = $jsonObject->js;
              
              echo $customScript;
//              echo $jsCode;
                
            } else {
                echo "No Script Code Found";
            }
        } else {
            echo "No Effect Model Found";
        }
    } else {
